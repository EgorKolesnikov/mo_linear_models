...
void LogisticRegressionModelForFtrlProximal::_update_w_one(DatasetEntry& entry){
...
void LogisticRegressionModelForFtrlProximal::_update_w(vector<DatasetEntry>& batch){
...
void LinearRegressionModelForFtrlProximal::_update_w_one(DatasetEntry& entry) {
    vector<vector<double> > z(this->_n_iterations, vector<double> (this->_n_features));
    vector<vector<double> > n(this->_n_iterations, vector<double> (this->_n_features));
    vector<vector<double> > w(this->_n_iterations, vector<double> (this->_n_features));
    for (int i = 0; i < this->_n_features; ++i) {
...
void LinearRegressionModelForFtrlProximal::_update_w(vector<DatasetEntry>& batch){
    _copy(this->w_cur, this->w_prev);
    for(DatasetEntry& entry : batch){
        this->_update_w_one(entry);
    }
